...
public class ModelTypeResource {
	private static final Logger logger = LoggerFactory.getLogger(ModelTypeResource.class);
	private static final String CONTENT_TYPE = "Content-Type";

	@Inject
	ModelTypeDAO repository;
	...
	public Response getFueltype() {
		...
		List<ModelType> entity = repository.query("ModelType.findAll");
		if (entity.size() != 0) {
	...
	public Response getFueltype(@PathParam("id") String id) {
		...
		List<ModelType> entity = repository.query(id);
		if (entity.size() != 0) {
			response = Response.ok().entity(entity).build();
		} else {
			response = Response.noContent().build();
	...
	public Response addFueltype(@QueryParam("name") String name)
		...
		logger.info("Início");
		ModelType novo = new ModelType(name);
	...
	public Response head() {
		logger.info("Início");
		Response retorno = Response.ok().header(ModelTypeResource.CONTENT_TYPE, MediaType.APPLICATION_JSON).build();
		logger.info("Fim");
		return retorno;
	}
