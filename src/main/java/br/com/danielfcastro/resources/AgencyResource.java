...
public class AgencyResource {
	...
	public Response getAgency(@PathParam("id") String id) {
		...
		List<Agency> entity = repository.query(id);
		if (entity.size() != 0) {
			response = Response.ok().entity(entity).build();
		} else {
			response = Response.noContent().build();
	...
	public Response updateAgency(@QueryParam("id") String id, @QueryParam("name") String name)
		...
		logger.info("Fim");
		return Response.ok("Agency updated with success!", MediaType.APPLICATION_JSON).build();
	}

	@DELETE
	@Path("/Agency/{id}")
	@Formatted
	public Response removeAgency(@PathParam("id") String id) {
		logger.info("Início");
		if (id == null || id.trim().length() == 0) {
			return Response.status(Response.Status.BAD_REQUEST).entity("id não pode ser nulo!").build();
		}
		...
		logger.info("Fim");
		return Response.ok("Agency removed with success!", MediaType.APPLICATION_JSON).build();
	...
	public Response head() {
		logger.info("Início");
		Response retorno = Response.ok().header(AgencyResource.CONTENT_TYPE, MediaType.APPLICATION_JSON).build();
		logger.info("Fim");
		return retorno;
	}
