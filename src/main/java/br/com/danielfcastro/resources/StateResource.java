...
public class StateResource {
	...
	public Response getFueltype(@PathParam("id") String id) {
		...
		List<State> entity = repository.query(id);
		if (entity.size() != 0) {
			response = Response.ok().entity(entity).build();
		} else {
			response = Response.noContent().build();
	...
	public Response updateFueltype(@PathParam("id") String id, @QueryParam("countryId") String countryId,
			@QueryParam("fu") String fu, @QueryParam("name") String name)
	...
	public Response head() {
		logger.info("Início");
		Response retorno = Response.ok().header(StateResource.CONTENT_TYPE, MediaType.APPLICATION_JSON).build();
		logger.info("Fim");
		return retorno;
	}
